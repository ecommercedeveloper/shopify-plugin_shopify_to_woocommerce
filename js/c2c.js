jQuery(document).ready(function($) {
  ...
  function checkApi(url,apiPass){
    $("#ajaxloader").show();
    result = $.ajax({
      cache: false,
      async: false,
      url: SELF_PATH + "checkApi&url="+encodeURIComponent(url)+"&apipass="+apiPass
    });
    var res;
  ...
  function createMigration()
  {
    ...
    $.ajax({
      ...
      data: {
        ...
        'sourceVariables': {'apiPassword':Cart2cartApiPass.val()},
      ...
      success: function(data){
        if (data.error == true){
          ...
          $('#startMigration').attr('onClick',
            "javascript: window.open('https://app.shopping-cart-migration.com/from-"
  ...
  sourceCartSetup.on("click",function(e){
    ...
    if (Cart2cartApiPass.val() == ''){
      ...
      $("#hostUser").html('Store API password is required');
      $("#hostUser").animate({
        'opacity': 'show'
      });
      hasError = true;
    ...
    if (!hasError && !checkApi(Cart2cartSourceUrl.val(), Cart2cartApiPass.val())){
      Cart2cartSourceUrl.html('Url or pasword is incorrect');
